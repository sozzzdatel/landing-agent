...
<p class="desc">${esc(t.discountPrefix)} ${esc(c.discount)} ${esc(t.discountMid)} <b>${esc(c.promo)}</b>.</p><a class="btn" href="${ref}" target="_blank" rel="noopener">${esc(c.cta)} ↗</a></div></section>`,
  ...
  // Порядок из анализа референса (только известные нам типы блоков), иначе — дефолтный порядок.
  let order=(c.styleOverride&&c.styleOverride.blocksOrder)?c.styleOverride.blocksOrder.filter(k=>blockHtml[k]):null;
  if(!order||!order.length) order=['stats','features','steps','faq','cta'];
  if(!order.includes('cta')) order.push('cta'); // финальный CTA всегда должен быть — иначе некуда вести трафик
