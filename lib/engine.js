...
function renderLanding(c, standalone){
  const b=BRANDS[c.brand]||BRANDS.studyai;
  // Если агент нашёл реальные цвета/шрифт сайта-референса — накладываем их поверх темы бренда.
  const v=Object.assign({}, b.vars, c.styleOverride?{
    acc:c.styleOverride.acc||b.vars.acc,
    acc2:c.styleOverride.acc2||b.vars.acc2,
    body:c.styleOverride.bodyFont||b.vars.body,
    radius:c.styleOverride.radius||b.vars.radius,
    ...(c.styleOverride.bg?(
      c.customMiddleHtml?{
        // ИИ сам пишет вёрстку под референс — доверяем ему целиком, красим всю страницу.
        // soft/line/mut — не точный bg, а полупрозрачные оверлеи, чтобы карточки и текст были видны на ЛЮБОМ фоне.
        bg:c.styleOverride.bg,
        soft:isLight(c.styleOverride.bg)?'rgba(0,0,0,.045)':'rgba(255,255,255,.08)',
        line:isLight(c.styleOverride.bg)?'rgba(0,0,0,.12)':'rgba(255,255,255,.18)',
        mut:isLight(c.styleOverride.bg)?b.vars.mut:'rgba(255,255,255,.72)',
        heroBg:c.styleOverride.bg2?`linear-gradient(135deg,${c.styleOverride.bg},${c.styleOverride.bg2})`:c.styleOverride.bg,
        heroInk:isLight(c.styleOverride.bg)?b.vars.ink:'#ffffff',
        ink:isLight(c.styleOverride.bg)?b.vars.ink:'#f5f5f5'
      }:{
        // Блочный движок — красим только hero, чтобы не портить читаемость остальных секций.
        heroBg:c.styleOverride.bg2?`linear-gradient(135deg,${c.styleOverride.bg},${c.styleOverride.bg2})`:c.styleOverride.bg,
        heroInk:isLight(c.styleOverride.bg)?b.vars.ink:'#ffffff'
      }
    ):{})
  ...
  const css=`:root{--acc:${v.acc};--acc2:${v.acc2};--acc-ink:${isLight(v.acc)?'#12101f':'#ffffff'};--ink:${v.ink};--bg:${v.bg};--soft:${v.soft};--line:${v.line};--mut:${v.mut};--radius:${v.radius}}
...
.btn{display:inline-flex;align-items:center;gap:8px;background:var(--acc);color:var(--acc-ink,#fff);font-weight:600;padding:14px 24px;border-radius:calc(var(--radius) - 4px);font-size:15px;transition:.2s;border:none;cursor:pointer}
...
.hero.center{text-align:center}.hero.center .hero-text{display:flex;flex-direction:column;align-items:center}.hero.center .row{justify-content:center}.hero.center p{margin-left:auto;margin-right:auto}
.hero.withimg .wrap{display:grid;grid-template-columns:1.1fr 0.9fr;gap:40px;align-items:center;text-align:left}
.hero.withimg.center .hero-text{align-items:flex-start;text-align:left}
.hero .visual{width:100%;border-radius:calc(var(--radius) + 6px);overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.2)}
.hero .visual img{width:100%;display:block}
@media(max-width:860px){.hero.withimg .wrap{grid-template-columns:1fr}.hero .visual{order:-1;max-width:320px;margin:0 auto 20px}}
...
.grid{display:grid;grid-template-columns:repeat(${(c.styleOverride&&c.styleOverride.featureColumns)||2},1fr);gap:18px;margin-top:40px}
...
footer{border-top:1px solid var(--line);padding:40px 0;color:var(--mut);font-size:14px}footer .wrap{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px}`;
  ...
  const spacious=(c.styleOverride&&c.styleOverride.spacious)?' spacious':'';
  const heroImgClass=c.heroImage?' withimg':'';
  const heroVisual=c.heroImage?`<div class="visual"><img src="${esc(c.heroImage)}" alt="" loading="lazy"></div>`:'';
  const heroClass=((c.styleOverride&&c.styleOverride.heroAlign==='center')?' center':'')+heroImgClass;

  // Средние блоки страницы — каждый доступен по имени, реальный порядок задаёт vision-анализ референса.
  const blockHtml={
    stats:`<div class="wrap"><div class="stats">${stats}</div></div>`,
    features:`<section id="tools" class="${spacious.trim()}"><div class="wrap"><div class="lead">${esc(t.leadWhat)}</div><h2 class="h2">${esc(c.toolsTitle||'')}</h2>
<p class="desc">${esc(c.toolsDesc||'')}</p><div class="grid">${tools}</div></div></section>`,
    steps:`<section class="steps${spacious}" id="how"><div class="wrap"><div class="lead">${esc(t.leadHow)}</div><h2 class="h2">${esc(t.howTitle)}</h2><div class="stepgrid">${steps}</div></div></section>`,
    faq:`<section id="faq" class="${spacious.trim()}"><div class="wrap"><div class="lead" style="text-align:center">${esc(t.leadFaq)}</div><h2 class="h2" style="text-align:center;margin:0 auto">${esc(t.faqTitle)}</h2><div class="faq">${faq}</div></div></section>`,
    cta:`<section class="final${spacious}"><div class="wrap"><h2 class="h2">${esc(c.finalTitle||'')}</h2>
<p class="desc">${esc(t.discountPrefix)} ${esc(c.discount)} ${esc(t.discountMid)} <b>${esc(c.promo)}</b>.</p><a class="btn" href="${ref}" target="_blank" rel="noopener">${esc(c.cta)} ↗</a></div></section>`,
  ...
  // Порядок из анализа референса — это ПРИОРИТЕТ показа, но полнота лендинга гарантирована:
  // все стандартные блоки всегда присутствуют, просто в порядке, увиденном на референсе.
  const known=['stats','features','steps','faq','cta'];
  let order=(c.styleOverride&&c.styleOverride.blocksOrder)?c.styleOverride.blocksOrder.filter(k=>blockHtml[k]):[];
  for(const k of known) if(!order.includes(k)) order.push(k); // добиваем недостающие стандартным порядком
  const middle=c.customMiddleHtml?c.customMiddleHtml:order.map(k=>blockHtml[k]).join('\n');
  const logoHtml=c.logoDataUrl?`<img src="${esc(c.logoDataUrl)}" alt="${esc(b.name)}" style="height:32px;max-width:160px;object-fit:contain">`:`<span class="logo">${esc(b.logo)}</span>`;

  const body=`<header><div class="wrap">${logoHtml}
...
<div class="hero${heroClass}"><div class="wrap"><div class="hero-text"><span class="eyebrow">${esc(c.eyebrow)}</span><h1>${esc(c.h1)}</h1><p>${esc(c.sub)}</p>
...
<div class="promo"><span class="big">${esc(c.discount)}</span><span>${esc(t.discountPrefix)} <code>${esc(c.promo)}</code> ${esc(t.discountMid)}</span></div></div>${heroVisual}</div></div>
${middle}
<footer><div class="wrap">${logoHtml}<a href="${ref}" target="_blank" rel="noopener">${esc(b.domain)} ↗</a></div></footer>`;
  ...
  const contrastFixScript=`<script>(function(){
function parseColor(str){var m=str&&str.match(/rgba?\\(\\s*(\\d+),\\s*(\\d+),\\s*(\\d+)/);return m?[+m[1],+m[2],+m[3]]:null}
...
function contrast(l1,l2){var a=Math.max(l1,l2)+0.05,b=Math.min(l1,l2)+0.05;return a/b}
function getBg(el){var node=el;while(node){var s=getComputedStyle(node),bg=s.backgroundColor,c=parseColor(bg);if(c&&bg!=='rgba(0, 0, 0, 0)')return c;node=node.parentElement}return [255,255,255]}
function fix(){
  document.querySelectorAll('h1,h2,h3,p,span,a,li,button,div').forEach(function(el){
    if(el.children.length>2)return;
    if(!el.textContent||!el.textContent.trim())return;
    var style=getComputedStyle(el),textColor=parseColor(style.color);
    if(!textColor)return;
    var bgColor=getBg(el);
    var ratio=contrast(lum(textColor),lum(bgColor));
    if(ratio<2.3){el.style.color=lum(bgColor)>0.5?'#14161c':'#ffffff'}
  });
}
if(document.readyState==='complete')fix();else window.addEventListener('load',fix);
})();</script>`;
  if(standalone)return `<!DOCTYPE html><html lang="${b.lang}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(b.name)} — ${esc(c.h1)}</title><link href="${fonts}" rel="stylesheet"><style>${css}</style></head><body>${body}${contrastFixScript}</body></html>`;
