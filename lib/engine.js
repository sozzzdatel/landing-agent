...
.hero.center{text-align:center}.hero.center .wrap{display:flex;flex-direction:column;align-items:center}.hero.center .row{justify-content:center}.hero.center p{margin-left:auto;margin-right:auto}
.hero.withimg .wrap{display:grid;grid-template-columns:1.1fr 0.9fr;gap:40px;align-items:center;text-align:left}
.hero.withimg.center .wrap{display:grid;text-align:left}
...
<div class="hero${heroClass}"><div class="wrap"><span class="eyebrow">${esc(c.eyebrow)}</span><h1>${esc(c.h1)}</h1><p>${esc(c.sub)}</p>
...
<div class="promo"><span class="big">${esc(c.discount)}</span><span>${esc(t.discountPrefix)} <code>${esc(c.promo)}</code> ${esc(t.discountMid)}</span></div></div>${heroVisual}</div>
${middle}
