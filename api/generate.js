...
- ГЛАВНОЕ: ты СМОТРИШЬ на реальный скриншот. Для КАЖДОЙ секции определяй её фон и цвет текста по тому, что реально видишь на этом участке картинки — не по общим переменным, не по формуле, а по своим глазам. Если на референсе один блок белый, следующий — тёмный, следующий — цветной, повтори это ТОЧНО: пиши explicit background-color (hex, который видишь) и явный color для текста этой секции, обеспечивая читаемый контраст ИМЕННО на этом фоне. Не переиспользуй один и тот же цвет фона для всех секций, если на референсе они разные.
- Кнопки/акценты (CTA, активные таблетки, важные цифры) — используй var(--acc) для фона кнопки и var(--acc-ink) для текста кнопки (эта переменная уже даёт правильный контраст автоматически).
- ЛЮБОЙ список из нескольких похожих пунктов (фичи, шаги, преимущества, тарифы, отзывы) ОБЯЗАН быть оформлен как визуальные карточки, никогда не голым текстом/списком. У карточки обязательно: явный background (несколько отличный от фона секции — светлее или темнее на глаз, как на референсе), border-radius (используй var(--radius)), padding не меньше 24px.
...
- Если есть таблетки-фильтры/теги — сделай их с явным фоном и текстом, обеспечивающим контраст на фоне их секции.
- Для заголовков блоков используй <h2 class="h2"> (даёт правильный адаптивный размер шрифта — не задавай свой font-size). Явно задай color на h2 под фон конкретной секции. Не используй <h1> нигде в середине страницы (h1 только один раз в hero).
...
- Всегда включи блок с призывом к действию (кнопка: <a href="${content.ref}" style="background:var(--acc);color:var(--acc-ink);padding:14px 24px;border-radius:calc(var(--radius) - 4px);display:inline-block;font-weight:600">) хотя бы один раз.
- Каждый крупный блок — <section style="padding:72px 0;background:...(явный цвет этой секции)..."><div style="max-width:1100px;margin:0 auto;padding:0 22px">...</div></section>.
